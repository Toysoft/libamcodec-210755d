...
{
    {ACODEC_FMT_AAC, "libfaad_aml.so"},
    {ACODEC_FMT_AAC_LATM, "libfaad_aml.so"},
    {ACODEC_FMT_APE, "libape_aml.so"},
    {ACODEC_FMT_MPEG, "libmad_aml.so"},
    {ACODEC_FMT_FLAC, "libflac_aml.so"},  
    {ACODEC_FMT_COOK, "libcook_aml.so"},
    {ACODEC_FMT_RAAC, "libraac_aml.so"},
    {ACODEC_FMT_AMR, "libamr_aml.so"},

    {ACODEC_FMT_PCM_S16BE,"libpcm_aml.so"},
    {ACODEC_FMT_PCM_S16LE,"libpcm_aml.so"},
    {ACODEC_FMT_PCM_U8,"libpcm_aml.so"},
    {ACODEC_FMT_PCM_BLURAY,"libpcm_aml.so"},
    {ACODEC_FMT_WIFIDISPLAY,"libpcm_aml.so"},
    {ACODEC_FMT_ALAW,"libpcm_aml.so"},
    {ACODEC_FMT_MULAW,"libpcm_aml.so"},
    {ACODEC_FMT_ADPCM,"libadpcm_aml.so"},
//   {ACODEC_FMT_AC3,"libmad_aml.so"},
//    {ACODEC_FMT_EAC3,"libmad_aml.so"},
    NULL
} ;
...
static int ioctl_get(int h, int subcmd, unsigned long paramter)
{
    int r;
    ...
    if (r < 0) {
        adec_print("ioctl_get failed,handle=%d,subcmd=%x,paramter=%x, t=%x\n", h, subcmd, paramter, r);
        return r;
    }
    if (paramter != 0) {
...
static int ioctl_set(int h, int subcmd, unsigned long paramter)
{
    int r;
    ...
    if (r < 0) {
        adec_print("ioctl_set failed,handle=%d,cmd=%x,paramter=%x, t=%x\n", h, subcmd, paramter, r);
        return r;
    }
    return 0;
...
#if defined (__aarch64__)
	    ioctl_get(dsp_ops->dsp_file_fd, AMSTREAM_GET_APTS_LOOKUP, &offset);
#else
...
#if defined (__aarch64__)
    ioctl_get(dsp_ops->dsp_file_fd, AMSTREAM_GET_PCRSCR, &val);
#else
...
{
	unsigned int val = apts;
    if (dsp_ops->dsp_file_fd < 0) {
        adec_print("armdec_set_apts err!\n");
        return -1;
    }
#if defined (__aarch64__)
    ioctl_set(dsp_ops->dsp_file_fd, AMSTREAM_SET_APTS, val);
#else
    ioctl(dsp_ops->dsp_file_fd, AMSTREAM_IOC_SET_APTS, val);
#endif
    return 0;
}
...
#if defined (__aarch64__)
    ioctl_get(dsp_ops->dsp_file_fd, AMSTREAM_GET_FIRST_APTS_FLAG, &val);
#else
...
static void start_adec(aml_audio_dec_t *audec)
{
    ...
    if (audec->state == INITTED) {
		...
		while ( (!audec->need_stop) && (!dsp_ops->get_cur_pts(dsp_ops)))
			usleep(100000);
               ...
               while (declen<rlen && !audec->exit_decode_thread) 
               {
                     ...
                     if(nAudioFormat == ACODEC_FMT_COOK || nAudioFormat == ACODEC_FMT_RAAC || nAudioFormat == ACODEC_FMT_AMR){
                      ...
                      if(nAudioFormat == ACODEC_FMT_RAAC ||nAudioFormat == ACODEC_FMT_COOK){
                        ...
                        if(0) { //(ACODEC_FMT_AC3 == nAudioFormat) || (ACODEC_FMT_EAC3 == nAudioFormat)) {
