...
int m_vpid, m_apid, m_pcrpid;
int m_vtype, m_atype;
CODEC_HANDLE aml_fd = -1;
CODEC_HANDLE cntl_fd = -1;
...
int aml_setAvsyncMode(int mode)
{
	int fd;
	char  bcmd[16] = { 0 };
	fd = open("/sys/class/tsync/mode", O_RDWR);
	if (fd >= 0) {
		sprintf(bcmd, "%d", mode);
...
int aml_open(void)
{
	m_vpid = m_apid = m_pcrpid = -1;
	m_vtype = m_atype = -1;
	
	memset(&am_param, 0, sizeof(arm_audio_info));
	memset(&am_sysinfo, 0, sizeof(dec_sysinfo_t));
	
	aml_fd = codec_h_open(CODEC_TS_DEVICE, O_RDWR);
    if (aml_fd < 0) {
		aml_fd = -1;
        return -1;
	}
		
	cntl_fd = codec_h_open(CODEC_CNTL_DEVICE, O_RDWR);
    if (cntl_fd < 0) {
		close(aml_fd);
		cntl_fd = aml_fd = -1;
    }
	codec_check_new_cmd(aml_fd);
    return CODEC_ERROR_NONE;
}
void aml_play(int vpid, int apid, int pcrpid, int vtype, int atype, int source)
{
	m_vpid = vpid;
	m_apid = apid;
	m_pcrpid = pcrpid;
	...
	aml_setAvsyncMode(source != 2 ? 2 : 1);
	
	if ((m_vpid >= 0) && (m_vpid < 0x1FFF))	 
	{									
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_VFORMAT, m_vtype);
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_VID, m_vpid);
		codec_h_control(aml_fd, AMSTREAM_IOC_SYSINFO, (unsigned long)&am_sysinfo);
	}
	if ((m_apid >= 0) && (m_apid < 0x1FFF)) 
	{
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_AFORMAT, m_atype);
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_AID, m_apid);
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_SAMPLERATE, 48000);
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_ACHANNEL, 2);
	}
	if ((m_pcrpid >= 0) && (m_pcrpid < 0x1FFF)) 
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_IOC_PCRID, m_pcrpid);
		
	/* Tell the kernel on which adapter we want HW CSA/LiveTV */
	codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_DEMUX, source);
	
	am_param.channels = 2;
	am_param.sample_rate = 48000;
	am_param.handle = aml_fd;
	am_param.format = m_atype;
	audio_start(&adec_handle, &am_param);
	
#if 0
	if (cntl_fd >= 0 && source != 2) 
	{
		codec_h_control(cntl_fd, AMSTREAM_IOC_AVTHRESH, (unsigned long)90000 * 30 );
		codec_h_control(cntl_fd, AMSTREAM_IOC_SYNCTHRESH, (unsigned long) 0);							
	}
#endif
	
	codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_PORT_INIT, 0);
	codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_TS_SKIPBYTE, 0);				
}
int aml_close(void)
{
	if (adec_handle)
		audio_stop(&adec_handle);
...
{
	if (adec_handle)
		 audio_pause(adec_handle);
		
	if ((m_vpid >= 0) && (m_vpid < 0x1FFF) && aml_fd >= 0)	 
		codec_h_control(cntl_fd, AMSTREAM_IOC_VPAUSE, 1);
}
void aml_resume(void)
{
	if (adec_handle)
		audio_resume(adec_handle);
		
	if ((m_vpid >= 0) && (m_vpid < 0x1FFF) && aml_fd >= 0)	 
		codec_h_control(cntl_fd, AMSTREAM_IOC_VPAUSE, 0);
}
void aml_change_apid(int apid, int atype)
{
	if (aml_fd >= 0)
	{
		m_apid = apid;
		...
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_AID, 0xffff);
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_AFORMAT, m_atype);
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_AID, m_apid);
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_SAMPLERATE, 48000);
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_SET_ACHANNEL, 2);
		codec_h_ioctl(aml_fd, AMSTREAM_IOC_SET, AMSTREAM_AUDIO_RESET, 0);
...
int aml_get_pts(void)
{
	...
	if (aml_fd >= 0 ) 
	{
		if (!codec_h_ioctl(aml_fd, AMSTREAM_IOC_GET, AMSTREAM_GET_PCRSCR, (unsigned long)&aml_pts));		
			return aml_pts;				
	}
	return -1;
}
int aml_setAvsyncEnable(int enable)
{
	int fd;
	char  bcmd[16] = { 0 };
	...
	enable = 1;
#endif	
	fd = open("/sys/class/tsync/enable", O_RDWR);
	if (fd >= 0) {
...
int aml_osdBlank(int cmd)
{
	int fd;
	char  bcmd[16] = { 0 };
	fd = open("/sys/class/video/blackout_policy", O_RDWR);
	if(fd >=0 ) {
		sprintf(bcmd,"%d",cmd);
		write(fd,bcmd,strlen(bcmd));
		close(fd);
